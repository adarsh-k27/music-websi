...
function MusicPlayer () {
  ...
  return (
    <div className='w-full h-[7rem] py-1 absolute bottom-10 left-0  flex flex-row justify-center items-center gap-3 z-auto bg-white'>
      <div
        className='mr-auto'
        onClick={e => {
          e.preventDefault()
      ...
      <div className='w-auto flex flex-col gap-1 items-start justify-start'>
        <p className='text-black/70 text-sm font-serif'>
          {allSongs[PlayingIndex]?.name.length > 20
            ? allSongs[PlayingIndex]?.name.splice(0, 20)
            : allSongs[PlayingIndex]?.name}
